...
shapefiles = [
    ...
    os.path.join(shapefile_dir, "medio_tocantins/medio_tocantins.shp"),
    os.path.join(shapefile_dir, "alto_tocantins/alto_tocantins.shp"),
    os.path.join(shapefile_dir, "tocantins/tocantins.shp")
]
...
# Image config
lon1, lon2, lat1, lat2 = -50.5, -45.5, -16.5, -4.5
...
def compute_monthly(acc_daily, spatial_avg_daily, dates,accumulate_vars, mean_vars):
    ...
    for i, date in enumerate(dates):
        ...
        for key, grids in acc_daily.items():
            monthly_acc[(key, ym)].append(grids[i])
        for key, values in spatial_avg_daily.items():
            monthly_spatial[(key, ym)].append(values[i])
...
def compute_annual(acc_daily, spatial_avg_daily, dates,accumulate_vars, mean_vars):
    ...
    for i, date in enumerate(dates):
        year = date.year
        for key, grids in acc_daily.items():
            annual_acc[(key, year)].append(grids[i])
        for key, values in spatial_avg_daily.items():
            annual_spatial[(key, year)].append(values[i])
...
def compute_climatology(acc_daily, spatial_avg_daily, dates,
    ...
    # 1. Agrupar dados por Ano e Mês para criar o "Total Mensal" primeiro
    ...
    for i, date in enumerate(dates):
        ...
        for key, grids in acc_daily.items():
            monthly_totals_acc[key][year_month].append(grids[i])
            
        for key, values in spatial_avg_daily.items():
            monthly_totals_spatial[key][year_month].append(values[i])

    # 2. Agora consolidamos os meses (Soma para chuva, Média para variáveis de estado)
    ...
    for key, year_month_dict in monthly_totals_acc.items():
        for (year, month), days_list in year_month_dict.items():
            ...
            if key in accumulate_vars:
                # Total acumulado deste mês específico (ex: Janeiro de 2011)
    ...
    for (key, month), month_values_list in climat_acc.items():
        res = np.nanmedian(np.stack(month_values_list), axis=0)
        # --- CORREÇÃO DO BORRÃO ---
        # Aplica a máscara apenas em Grids (2D) para evitar erro com o FLOW (1D)
        if mask_nan is not None and res.ndim == 2:
...
if mohid == 1:
    for date_folder in sorted(os.listdir(root_dir)):
        ...
        hdf_flow = os.path.join(folder_path, "DrainageNetwork_1.hdf5")
...
# ====================== WATER BALANCE =================
# ======================================================
# Criar a lista de datas diárias com base no comprimento de PM (ou outra variável que tem todos os dias)
dates_list = []
